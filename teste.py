...
# Modificação para os operadores
def operador(num):
    global conta, label
...
# Captura de teclas do teclado físico com PyAutoGUI
...
# Definindo os botões
n0 = Button(frame, text='0', font='Arial 12 bold', width=6, height=2, command=lambda: numeros(0))
n0.grid(row=4, column=0)
ponto = Button(frame, text='.', font='Arial 12 bold', width=6, height=2, command=lambda: funcPonto())
ponto.grid(row=4, column=1)
soma = Button(frame, text='+', font='Arial 12 bold', width=6, height=2, command=lambda: operador('+'))
...
# Configuração do evento de teclado físico
